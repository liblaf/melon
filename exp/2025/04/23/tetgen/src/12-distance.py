...
class Config(cherries.BaseConfig):
    tetmesh: Path = grapes.find_project_dir() / "data/03-primary/tetgen.vtu"
    output: Path = grapes.find_project_dir() / "data/03-primary/tetgen-distance.vtp"


def main(cfg: Config) -> None:
    tetmesh: pv.UnstructuredGrid = melon.load(cfg.tetmesh)
    surface: pv.PolyData = tetmesh.extract_surface()
    ...
    path: Integer[np.ndarray, " P"] = edges.point_data["point-id"][path]
